...
A implementação abordou os seguintes pontos:
...
A Tabela~\ref{tab:melhores-parametros} mostra a configuração com maior Silhouette médio do ClonalG em cada caso.
...
A Figura~\ref{fig:comparativo-visual-clonalg-kmeans} mostra visualmente os agrupamentos gerados pelo ClonalG e pelo KMeans. Para cada dataset, a coluna da esquerda apresenta o agrupamento obtido pelo ClonalG, enquanto a coluna da direita apresenta o agrupamento obtido pelo KMeans. Os pontos representam as amostras e os marcadores pretos representam os centroides usados por cada algoritmo.

Nos datasets com dois atributos, a visualização usa os próprios atributos padronizados. Nos datasets com mais de dois atributos, a figura usa uma projeção PCA em duas dimensões apenas para facilitar a visualização. Essa projeção não altera o cálculo do Silhouette nem os resultados numéricos apresentados nas tabelas.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.86\textheight,keepaspectratio]{resultados/comparativo_visual_clonalg_kmeans.png}
\caption{Comparação visual dos agrupamentos gerados pelo ClonalG e pelo KMeans.}
\label{fig:comparativo-visual-clonalg-kmeans}
\end{figure}

A comparação visual ajuda a interpretar os resultados numéricos. Nos DataSets 1 e 5, os agrupamentos aparecem estáveis e semelhantes entre os dois algoritmos, o que combina com o empate no Silhouette. No DataSet 2, o ClonalG apresentou separação mais favorável segundo a métrica usada. No DataSet 3, a visualização reforça que a separação é mais difícil, e o KMeans obteve vantagem numérica. No DataSet 4, os dois algoritmos produziram agrupamentos próximos, com pequena vantagem para o ClonalG na melhor configuração.
...
A análise dos parâmetros mostrou que população maior e maior substituição podem melhorar a exploração, mas também aumentam custo computacional. A comparação visual dos agrupamentos reforça os resultados numéricos: em alguns datasets os dois algoritmos chegam a partições muito parecidas, enquanto em outros a busca populacional do ClonalG encontra uma organização mais favorável segundo o Silhouette.
